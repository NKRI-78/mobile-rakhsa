...
class CreateStoreOrUpdateScreen extends StatefulWidget {
  const CreateStoreOrUpdateScreen({super.key});

  @override
  State<CreateStoreOrUpdateScreen> createState() =>
      CreateStoreOrUpdateScreenState();
}
...
class CreateStoreOrUpdateScreenState extends State<CreateStoreOrUpdateScreen> {
  ...
  Future<void> submit() async {
    String id = storeId == "" ? const Uuid().v4() : storeId;
  ...
  String capitalizeEachWord(String input) {
    // Pisahkan string menjadi daftar kata menggunakan spasi sebagai pemisah
    List<String> words = input.split(' ');

    // Ubah setiap kata menjadi huruf kapital di awal
    List<String> capitalizedWords = words.map((word) {
      if (word.isEmpty) return word; // Abaikan kata kosong
  ...
  void initState() {
    ...
    emailC = TextEditingController(
        text: profileProvider.entity.data!.email.toString());
    phoneC = TextEditingController(
        text: profileProvider.entity.data!.contact.toString());
    ...
    Future.delayed(Duration.zero, () async {
      try {
        ...
        setState(() {
          ...
          nameStoreC =
              TextEditingController(text: isLoading ? "" : store?.name ?? "");
          descStoreC = TextEditingController(
              text: isLoading ? "" : store?.description ?? "");
          provinceC = TextEditingController(
              text: isLoading ? "" : store?.province ?? "");
          cityC =
              TextEditingController(text: isLoading ? "" : store?.city ?? "");
          districtC = TextEditingController(
              text: isLoading ? "" : store?.district ?? "");
          subdistrictC = TextEditingController(
              text: isLoading ? "" : store?.subdistrict ?? "");
          postCodeC = TextEditingController(
              text: isLoading ? "" : store?.postalCode ?? "");
          addressC = TextEditingController(
              text: isLoading ? "" : store?.address ?? "");
          emailC =
              TextEditingController(text: isLoading ? "" : store?.email ?? "");
          phoneC =
              TextEditingController(text: isLoading ? "" : store?.phone ?? "");
                                  ...
                                  placeholder:
                                    ...
                                    return const CircleAvatar(
                                      ...
                                      child: Icon(Icons.store,
                                          size: 80.0, color: Color(0xFFC82927)),
                                    );
                                  },
        ...
        child: Card(
            ...
            child:
              ...
              Container(
                ...
                child: Text(
                    location.isEmpty ? "Location no Selected" : location,
                    style: robotoRegular.copyWith(
                        color: ColorResources.black,
                        fontSize: Dimensions.fontSizeDefault)),
              ),
                                            ...
                                            return ListTile(
                                              ...
                                              onTap: () async {
                                                setState(() {
                                                  ...
                                                  subdistrictC.text =
                                                      subdistrictName;
                                                  postCodeC.text = notifier
                                                      .subdistrict[i].zipCode
                                                      .toString();
                                                });
                                                Navigator.pop(context);
                                              },
  ...
  Widget toggleIsOpen() {
    return SwitchListTile(
      title: Text(
        'Buka Toko',
        style: robotoRegular.copyWith(
            fontSize: Dimensions.fontSizeDefault, color: ColorResources.black),
      ),
      ...
      children: [
        ...
        InkWell(
          ...
          child: Container(
              ...
              child: Text(descStoreC.text == '' ? "" : descStoreC.text,
                  style: robotoRegular.copyWith(
                      fontSize: Dimensions.fontSizeDefault,
