...
class EventRemoteDataSourceImpl implements EventRemoteDataSource {
  ...
  Future<EventModel> list() async {
    try {
      ...
      final response = await client.post(
        "${RemoteDataSourceConsts.baseUrlProd}/api/v1/event/list-event-user",
  ...
  Future<EventDetailModel> detail({required int id}) async {
    try {
      final response = await client.get(
        "${RemoteDataSourceConsts.baseUrlProd}/api/v1/event/$id",
      );
      Map<String, dynamic> data = response.data;
  ...
  Future<void> save({
    ...
    try {
      final session = await StorageHelper.getUserSession();
      await client.post(
        "${RemoteDataSourceConsts.baseUrlProd}/api/v1/event",
        data: {
          "title": title,
